package com.jusipat.astikorcartsredux.client.renderer.texture;
            ...
            for (int dy = 0; dy < r; dy++) {
                for (int x = this.x * resolution; x < x1; x++) {
                    for (int dx = 0; dx < r; dx++) {
                        ...
                        final int rgba = sprite.contents().originalImage.getPixel(Math.floorMod(u1, sprite.contents().width()), Math.floorMod(v1, sprite.contents().height()));
                        image.setPixel(r * x + dx, r * y + dy, rgba);
                    }
                }
            }
        }
