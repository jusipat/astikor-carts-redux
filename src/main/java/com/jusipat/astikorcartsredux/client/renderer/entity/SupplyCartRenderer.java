    ...
    public void renderSupplies(final AbstractCargoCart entity, final PoseStack stack, final MultiBufferSource source, final int packedLight, final NonNullList<ItemStack> cargo) {
        ...
        for (int i = 0; i < cargo.size(); i++) {
            ...
            if (model.isGui3d() && itemStack.getItem() != Items.TRIDENT && AstikorCartsReduxConfig.getClient().renderSupplyGear.get()) {
                ...
                if ((ArmorItem.class.equals(itemStack.getItem().getClass()) || DyeableArmorItem.class.equals(itemStack.getItem().getClass())) && AstikorCartsReduxConfig.getClient().renderSupplyGear.get()) {
