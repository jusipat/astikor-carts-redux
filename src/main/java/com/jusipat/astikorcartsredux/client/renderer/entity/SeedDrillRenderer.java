...
public class SeedDrillRenderer extends DrawnRenderer<SeedDrillEntity, SeedDrillRenderState, SeedDrillModel> {
    ...
    public @NotNull ResourceLocation getTextureLocation(final SeedDrillRenderState state) {
        return AstikorCartsRedux.resLoc("textures/entity/" + state.woodType.name() + "_seed_drill.png");
    }

    @Override
    protected void renderContents(SeedDrillRenderState state, final PoseStack stack, final MultiBufferSource source, final int packedLight) {
        for (int i = 0; i < state.seeds.size(); i++) {
            ...
            for (int j = 0; j < itemStack.getCount() / 4; j++) {
                ...
                this.attach(this.model.getBody(), this.model.getBody(), s -> {
                    ...
                    Minecraft.getInstance().getItemRenderer().renderStatic(itemStack, ItemDisplayContext.FIXED, packedLight, OverlayTexture.NO_OVERLAY, s, source, state.level, 0);
                }, stack);
            }
        }
    }
