...
import com.jusipat.astikorcartsredux.util.NiftyGoalAdder;
...
public class AstikorCartsRedux {
    ...
    public static final DeferredRegister.Entities ENTITY_TYPES = DeferredRegister.createEntities(AstikorCartsRedux.MODID);
    ...
    // Creates a new Block with the id "astikorcartsredux:example_block", combining the namespace and path
    public static final DeferredBlock<Block> EXAMPLE_BLOCK = BLOCKS.registerSimpleBlock("example_block", BlockBehaviour.Properties.of().mapColor(MapColor.STONE));
    // Creates a new BlockItem with the id "astikorcartsredux:example_block", combining the namespace and path
    public static final DeferredItem<BlockItem> EXAMPLE_BLOCK_ITEM = ITEMS.registerSimpleBlockItem("example_block", EXAMPLE_BLOCK);

    // Creates a new food item with the id "astikorcartsredux:example_id", nutrition 1 and saturation 2
    ...
    private static final TriFunction<WoodType, String, FeatureFlag[], DeferredItem<CartItem>> CART_ITEM_SUPPLIER =
            (wood, type, flags) ->
                    ITEMS.register(
                            wood.name() + "_" + type,
                            () -> new CartItem(
    ...
    public static final Map<String, Map<WoodType, DeferredItem<CartItem>>> CARTS = new HashMap<>();
    ...
    static {
        ...
        for (String type : CART_TYPES) {
            Map<WoodType, DeferredItem<CartItem>> perWoodMap = new HashMap<>();

            for (WoodType wood : VANILLA_WOOD_TYPES) {
                String registryName = wood.name() + "_" + type;

                DeferredItem<CartItem> item = ITEMS.register(
                        registryName,
                        () -> new CartItem(
                                wood,
                                type,
                                new Item.Properties().stacksTo(1).requiredFeatures(flags).setId(ResourceKey.create(Registries.ITEM, resLoc(registryName))))
    ...
    public static final Supplier<EntityType<SupplyCartEntity>> SUPPLY_CART_ENTITY = ENTITY_TYPES.registerEntityType(
            "supply_cart", SupplyCartEntity::new, MobCategory.MISC,
            builder -> builder.sized(1.5f, 1.4f));

    public static final Supplier<EntityType<AnimalCartEntity>> ANIMAL_CART_ENTITY = ENTITY_TYPES.registerEntityType(
            "animal_cart", AnimalCartEntity::new, MobCategory.MISC,
            builder -> builder.sized(1.3f, 1.4f));

    public static final Supplier<EntityType<PlowEntity>> PLOW_ENTITY = ENTITY_TYPES.registerEntityType(
            "plow", PlowEntity::new, MobCategory.MISC,
            builder -> builder.sized(1.3f, 1.4f));

    public static final Supplier<EntityType<HandCartEntity>> HAND_CART_ENTITY = ENTITY_TYPES.registerEntityType(
            "hand_cart", HandCartEntity::new, MobCategory.MISC,
            builder -> builder.sized(1.3f, 1.1f));

    public static final Supplier<EntityType<SeedDrillEntity>> SEED_DRILL_ENTITY = ENTITY_TYPES.registerEntityType(
            "seed_drill", SeedDrillEntity::new, MobCategory.MISC,
            builder -> builder.sized(1.3f, 1.4f));

    public static final Supplier<EntityType<ReaperCartEntity>> REAPER_ENTITY = ENTITY_TYPES.registerEntityType(
            "reaper", ReaperCartEntity::new, MobCategory.MISC,
            builder -> builder.sized(1.3f, 1.4f));

    public static final Supplier<EntityType<PostilionEntity>> POSTILION_ENTITY = ENTITY_TYPES.registerEntityType(
            "postilion", PostilionEntity::new, MobCategory.MISC,
            builder -> builder.sized(0.25f, 0.25f).noSummon().noSave());
