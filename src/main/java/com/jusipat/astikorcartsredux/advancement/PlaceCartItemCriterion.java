...
public class PlaceCartItemCriterion extends SimpleCriterionTrigger<PlaceCartItemCriterion.TriggerInstance> {
    ...
    public record TriggerInstance(Optional<ContextAwarePredicate> player, Optional<ItemPredicate> item) implements SimpleInstance {
        ...
        public static Criterion<TriggerInstance> placeCart() {
            return ACCriteriaTriggers.PLACE_CART_ITEM.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.empty()));
        }

        public static Criterion<TriggerInstance> placeCart(CartItem item) {
            return placeCart(ItemPredicate.Builder.item().of(item));
        }

        public static Criterion<TriggerInstance> placeCart(ItemPredicate.Builder builder) {
            return ACCriteriaTriggers.PLACE_CART_ITEM.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.of(builder.build())));
        }
