...
public class UsePlowCriterion extends SimpleCriterionTrigger<UsePlowCriterion.TriggerInstance> {
    ...
    public record TriggerInstance(Optional<ContextAwarePredicate> player, Optional<ItemPredicate> item) implements SimpleInstance {
        ...
        public static Criterion<TriggerInstance> usePlow() {
            return ACCriteriaTriggers.USE_PLOW.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.empty()));
        }

        public static Criterion<TriggerInstance> usePlow(ItemPredicate.Builder builder) {
            return ACCriteriaTriggers.USE_PLOW.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.of(builder.build())));
        }
