...
public class PullCartCriterion extends SimpleCriterionTrigger<PullCartCriterion.TriggerInstance> {
    ...
    public record TriggerInstance(Optional<ContextAwarePredicate> player, Optional<EntityTypePredicate> type, Optional<Float> minDist, Optional<Integer> minPassengerCount, Optional<Float> fillLevel) implements SimpleInstance {
        ...
        public static Criterion<TriggerInstance> pullCart(EntityTypePredicate type) {
            return ACCriteriaTriggers.PULL_CART.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.of(type), Optional.empty(), Optional.empty(), Optional.empty()));
        }

        public static Criterion<TriggerInstance> pullCartFill(EntityTypePredicate type, float fillLevel) {
            return ACCriteriaTriggers.PULL_CART.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.of(type), Optional.empty(), Optional.empty(), Optional.of(fillLevel)));
        }

        public static Criterion<TriggerInstance> pullCartDist(EntityTypePredicate type, float minDist) {
            return ACCriteriaTriggers.PULL_CART.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.of(type), Optional.of(minDist), Optional.empty(), Optional.empty()));
        }

        public static Criterion<TriggerInstance> pullCart(EntityTypePredicate type, float minDist, int minPassengerCount) {
            return ACCriteriaTriggers.PULL_CART.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.of(type), Optional.of(minDist), Optional.of(minPassengerCount), Optional.empty()));
        }
