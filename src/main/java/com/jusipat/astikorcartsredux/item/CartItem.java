...
public class CartItem extends Item {
    ...
    public @NotNull InteractionResult use(Level level, Player player, InteractionHand interactionHand) {
        ...
        if (result.getType() == HitResult.Type.MISS) {
            ...
            final List<Entity> list = level.getEntities(
                    player,
                    player.getBoundingBox().expandTowards(lookVec.scale(5.0D)).inflate(5.0D),
                    EntitySelector.NO_SPECTATORS.and(e -> e.canBeCollidedWith(player))
            );
            if (!list.isEmpty()) {
            ...
            if (result.getType() == HitResult.Type.BLOCK) {
                ResourceKey<EntityType<?>> key = ResourceKey.create(Registries.ENTITY_TYPE, AstikorCartsRedux.resLoc(cartType));
