            ...
            for (int dy = 0; dy < r; dy++) {
                for (int x = this.x * resolution; x < x1; x++) {
                    for (int dx = 0; dx < r; dx++) {
                        ...
                        final int rgba = sprite.contents().getOriginalImage().getLuminanceOrAlpha(Math.floorMod(u1, sprite.contents().width()), Math.floorMod(v1, sprite.contents().height()));
                        //final int rgba = sprite.getPixelRGBA(0, Math.floorMod(u1, sprite.contents().width()), Math.floorMod(v1, sprite.contents().height()));;
                        image.getPixel(r * x + dx, r * y + dy);
                    }
                }
            }
        }
