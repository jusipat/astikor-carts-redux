...
public class AstikorWorld extends SavedData {
    ...
    public static AstikorWorld getServer(MinecraftServer server, ResourceKey<Level> levelType) {
        ...
        Factory<AstikorWorld> factory = new Factory<>(
                AstikorWorld::new,
                (tag, provider) -> AstikorWorld.createFromNbt(tag, server.getLevel(levelType)),
                null
        );
        return dataStorage.computeIfAbsent(factory, AstikorCarts.ID);
    }
}